...
@router.get("/itunes")
async def get_properties_itunes(filter_query: Annotated[ItunesFilterParams, Query()]) -> List[Union[ItunesApiSongModel, ItunesApiAlbumKeys]]:
    """
...
async def put_properties(
    ...
    downloaded_song: Optional[DownloadCachedSong] = await db.hgetall(config.redis_song_id_prefix, body.song_id, DownloadCachedSong) # pyright: ignore
    ...
    await db.hset(config.redis_song_id_prefix, body.song_id, downloaded_song.model_dump())
    ...
    res = await db.index(config.redis_song_index_prefix, body.song_id, cached_song.model_dump())
