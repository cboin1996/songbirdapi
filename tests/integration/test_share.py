...
async def test_create_share_link(test_client: AsyncClient, regular_user, sample_song):
    ...
    resp = await test_client.post(f"/v1/share/songs/{sample_song.uuid}", cookies=cookies)
...
async def test_create_share_link_nonexistent_song(test_client: AsyncClient, regular_user):
    cookies = await login(test_client, regular_user)
